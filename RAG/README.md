...
![RAG 파이프라인](docs/images/rag-flow.svg)
...
- `src/v1_no_retrieval.py` — **검색 없는 최소 RAG**. `data/`의 PDF를 PyMuPDF로
  텍스트 추출해 통째로 프롬프트에 넣고 LLM에 묻는다. 벡터 검색·임베딩 없음. 문서가
  적을 때만 유효하며, 문서를 늘리면 토큰이 터지는 것을 직접 겪어 v2(벡터 검색)의
  필요성을 체감한다. (PDF 파싱 라이브러리 비교는 PyMuPDF 채택 — 속도·정확도 균형,
  단 AGPL 라이선스라 상용 시 주의.)

```bash
uv run python src/v1_no_retrieval.py            # self-check (API 키 불필요)
ANTHROPIC_API_KEY=... uv run python src/v1_no_retrieval.py "질문"
```
