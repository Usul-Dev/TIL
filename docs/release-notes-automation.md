...
릴리즈 노트에는 `release_date`가 속한 ISO 주의 커밋만 포함한다.

- 시작: 해당 ISO 주의 월요일 `00:00:00`
- 종료: 다음 ISO 주의 월요일 `00:00:00` 직전
- 기준 시간대: `Asia/Seoul`

예를 들어 `release_date`가 `2026-05-24`면 `2026-05-18T00:00:00+09:00`부터 `2026-05-25T00:00:00+09:00` 전까지의 커밋만 포함한다.

GitHub Release 태그는 릴리즈 노트에 포함된 마지막 커밋을 가리킨다. 따라서 과거 주차를 나중에 수동 생성해도 현재 `HEAD`가 과거 릴리즈 태그에 잘못 묶이지 않는다.

## Release Note Format

릴리즈 노트는 변경된 최상위 디렉터리별로 커밋을 나열한다. 예를 들어 `레디스/**`만 변경되면 `레디스`만 노출하고, 변경이 없는 대목은 표시하지 않는다.

```markdown
## 레디스

- `147114e` fix: 직접 실행 진입점 제거
- `371593a` docs: 커밋 메시지 규칙 정리
```

기본적으로 `.github`, `.idea`, `scripts`, `docs`는 릴리즈 노트 주제에서 제외한다. 필요하면 workflow 또는 로컬 실행에서 `RELEASE_NOTE_IGNORED_TOPICS`로 바꿀 수 있다.
...
- `base_ref`: 비우면 직전 `vYYYY-Wxx` 태그부터 비교한다. 날짜 범위 필터는 항상 적용된다.
